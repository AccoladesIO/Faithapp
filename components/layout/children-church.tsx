...
function WorkerOnlyMessage({ tabName }: { tabName: string }) {
    return (
    ...
    return (
        <div className="space-y-5 text-center">
            ...
            <div className="flex gap-3">
                <button
                    onClick={() => { navigator.clipboard.writeText(result.pickupCode); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                ...
                <button onClick={onDone} className="flex-1 bg-[#121212] text-white text-xs uppercase tracking-widest font-semibold py-3 rounded-xl hover:bg-gray-800 transition-colors">
                    Done
                </button>
            </div>
        </div>
                    ...
                    <div className="bg-white border border-[#121212]/5 rounded-2xl p-4">
                        ...
                        {editNotes ? (
                            <div className="space-y-2">
                                <textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Allergies, conditions…" className={`${inputCls} resize-none`} />
                                <div className="flex gap-2">
                                    <button onClick={() => setEditNotes(false)} className="flex-1 text-xs uppercase tracking-widest font-semibold py-2 rounded-lg border border-[#121212]/10 text-gray-500 hover:text-[#121212] transition-colors">Cancel</button>
                                    <button onClick={async () => { await hook.updateChild(child.id, { specialNotes: notes }); setEditNotes(false); }} disabled={hook.isSubmitting}
                    ...
                    {!showGForm ? (
                        ...
                        <form onSubmit={handleAddGuardian} className="bg-white border border-[#121212]/10 rounded-2xl p-4 space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <Field label="Phone"><input type="tel" placeholder="+234…" value={gForm.phoneNumber} onChange={(e) => setGForm((p) => ({ ...p, phoneNumber: e.target.value }))} className={inputCls} /></Field>
                            </div>
                            <Field label="Email (Optional)"><input type="email" placeholder="email@example.com" value={gForm.email} onChange={(e) => setGForm((p) => ({ ...p, email: e.target.value }))} className={inputCls} /></Field>
                            <div className="flex items-center justify-between bg-[#F9F9F9] rounded-xl px-3 py-2.5">
                                ...
                                <button type="button" onClick={() => setGForm((p) => ({ ...p, isAuthorizedPickup: !p.isAuthorizedPickup }))}
                            ...
                            <button type="submit" disabled={hook.isSubmitting} className="w-full bg-[#121212] text-white text-xs uppercase tracking-widest font-semibold py-2.5 rounded-xl hover:bg-gray-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <div className="bg-white border border-[#121212]/10 rounded-2xl p-5 space-y-4">
                    ...
                    <form onSubmit={handleCreate} className="space-y-3">
                        ...
                        <Field label="Special Notes (Optional)">
                            <textarea rows={2} placeholder="Allergies, conditions…" value={form.specialNotes} onChange={(e) => setForm((p) => ({ ...p, specialNotes: e.target.value }))} className={`${inputCls} resize-none`} />
                        </Field>
                        {hook.submitError && <ErrorBanner message={hook.submitError} />}
                        <button type="submit" disabled={hook.isSubmitting} className="w-full bg-[#121212] text-white text-xs uppercase tracking-widest font-semibold py-3 rounded-xl hover:bg-gray-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
...
function CheckInTab({
    ...
    const [selectedChildId, setChildId] = useState("");
    const [selectedGuardianId, setGuardianId] = useState("");
    const [serviceSlotId, setSlotId] = useState("");
    const [childGuardians, setChildGuardians] = useState<Guardian[]>([]);
    const [loadingGuardians, setLoadingG] = useState(false);
    const [result, setResult] = useState<CheckinResult | null>(null);

    const selectedChild = children.find((c) => c.id === selectedChildId) ?? null;
    const selectedGuardian = childGuardians.find((g) => g.id === selectedGuardianId) ?? null;

    // Flatten all service slots from all events for slot selector
    const allSlots = events.flatMap((e) =>
        e.serviceSlots.map((s) => ({ ...s, eventName: e.name }))
    );

    // Load guardians when child changes
    useEffect(() => {
        if (!selectedChildId) { setChildGuardians([]); setGuardianId(""); return; }
        setLoadingG(true);
        hook.getGuardians(selectedChildId)
            .then(setChildGuardians)
            .finally(() => setLoadingG(false));
        setGuardianId("");
    }, [selectedChildId, hook]);

    if (!isWorker) return <WorkerOnlyMessage tabName="Check-In" />;

    const handleCheckin = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedChildId || !selectedGuardianId || !serviceSlotId) return;
        try {
            const res = await hook.checkin({
                childId: selectedChildId,
                serviceSlotId,
                droppedOffByGuardianId: selectedGuardianId,
                droppedOffByName: selectedGuardian?.fullName ?? `${selectedGuardian?.firstname ?? ""} ${selectedGuardian?.lastname ?? ""}`.trim(),
            });
            setResult(res);
            setStep("code");
        } catch { /* error in hook */ }
    ...
    if (step === "code" && result) return <PickupCodeCard result={result} onDone={handleDone} />;

    return (
        <div className="space-y-5">
            <div className="bg-[#F4F1EA]/50 border border-[#121212]/5 rounded-2xl p-4">
                <h4 className="text-xs font-semibold text-[#121212] mb-1">Children's Church Check-In</h4>
                <p className="text-xs text-gray-500 font-light leading-relaxed">
                    Select the child, the guardian dropping them off, and the service slot. A unique pickup code will be generated.
                </p>
            </div>
            ...
            {children.length === 0 ? (
                ...
                <form onSubmit={handleCheckin} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            {children.map((child) => (
                                <button key={child.id} type="button" onClick={() => setChildId(child.id)}
                                    className={`w-full flex items-center gap-3 p-3.5 rounded-xl border transition-all text-left ${selectedChildId === child.id ? "bg-[#EADCC9] border-[#8A817C]/30 shadow-sm" : "bg-white border-[#121212]/5 hover:border-[#121212]/10"}`}>
                    ...
                    {/* Guardian selector — loaded from child's registered guardians */}
                    {selectedChildId && (
                        <Field label="Dropped Off By (Guardian)">
                            {loadingGuardians ? (
                                ...
                                <div className="bg-amber-50 border border-amber-100 rounded-xl px-3 py-3 text-xs text-amber-700">
                                    No guardians registered for this child. Add one in My Children → Guardians.
                                </div>
                            ) : (
                                <div className="space-y-2">
                    ...
                    <button type="submit"
                        disabled={hook.isSubmitting || !selectedChildId || !selectedGuardianId || !serviceSlotId || childGuardians.length === 0}
...
function CheckOutTab({
    ...
    if (!isWorker) return <WorkerOnlyMessage tabName="Check-Out" />;

    const selectedGuardian = verified?.authorizedGuardians.find((g) => g.id === selectedGuardianId) ?? null;
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setVerifyError(err instanceof Error ? err.message : "Invalid or expired pickup code.");
        } finally {
            setVerifying(false);
        }
    };
    ...
    const handleCheckout = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedGuardianId || !selectedGuardian) return;
        try {
            await hook.checkout({
                pickupCode: codeInput.trim().toUpperCase(),
                pickedUpByGuardianId: selectedGuardianId,
                pickedUpByName: selectedGuardian.fullName
            ...
            {!verified ? (
                <form onSubmit={handleVerify} className="space-y-4">
                    ...
                    <Field label="Pickup Code">
                        <input type="text" required maxLength={6} placeholder="A7K3"
                            value={codeInput} onChange={(e) => setCode(e.target.value.toUpperCase())}
                    ...
                    <button type="submit" disabled={verifying || codeInput.length < 3}
                        ...
                        {verifying ? <><Loader2 size={14} className="animate-spin" /> Verifying…</> : <><ShieldCheck size={14} /> Verify Code</>}
                    </button>
                </form>
            ) : (
                <div className="space-y-4">
                    ...
                    <div className="bg-green-50 border border-green-100 rounded-2xl p-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium text-green-900 mt-0.5">{verified.child.firstname} {verified.child.lastname}</p>
                        </div>
                    </div>

                    {/* Guardian selector from authorizedGuardians */}
                    <Field label="Select Collecting Guardian">
                        {verified.authorizedGuardians.length === 0 ? (
                            <div className="bg-amber-50 border border-amber-100 rounded-xl px-3 py-3 text-xs text-amber-700">
                                No authorised guardians on record for this child.
                            </div>
                        ) : (
                            <div className="space-y-2">
                    ...
                    <form onSubmit={handleCheckout} className="flex gap-3">
                        <button type="button" onClick={() => { setVerified(null); setCode(""); setGuardianId(""); }}
                        ...
                        <button type="submit"
                            disabled={hook.isSubmitting || !selectedGuardianId}
            ...
            <div className="px-5 mt-6">
                {activeTab === "children" && (
                    selectedChild ? (
                        ...
                        <MyChildrenTab hook={hook} memberId={memberId} children={children} setChildren={setChildren} onChildSelect={setSelected} />
                    )
                )}
