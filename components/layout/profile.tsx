...
import { useAnnouncementPush } from "@/hooks/use-announcement-push";
        ...
        <form onSubmit={handleSubmit} className="pt-3 space-y-4">
            ...
            <div>
                ...
                <input
                    type="text"
                    required
                    placeholder="Broken equipment in hall B"
                    value={form.title}
            ...
            <div>
                ...
                <input
                    type="text"
                    required
                    placeholder="Hall B, Main Auditorium…"
                    value={form.location}
                ...
                {images.length < MAX_IMAGES && (
                    <label className="flex items-center gap-2 w-full bg-white border border-dashed border-[#121212]/15 rounded-xl px-3 py-3 text-xs cursor-pointer hover:border-[#121212]/30 transition-colors">
                        ...
                        <span className="text-gray-400">
                            {images.length === 0
                                ? "Add photos of the incident…"
                                : `Add more (${images.length}/${MAX_IMAGES} selected)`}
                        </span>
                        <input
                            ref={fileInputRef}
                            type="file"
            ...
            <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                        ...
                        {isFull && (
                            <span className="text-[8px] uppercase tracking-wider font-bold bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">Full</span>
                        )}
                        {meeting.selectionStatus === "OPEN" && !isFull && (
                            <span className="text-[8px] uppercase tracking-wider font-bold bg-green-50 text-green-700 px-1.5 py-0.5 rounded">Open</span>
                        )}
                    </div>
                    <div className="flex items-center gap-3 text-[10px] text-gray-400 font-light">
                        <span className="flex items-center gap-1">
                            <Clock size={10} /> {dayConfig.startTime} – {dayConfig.endTime}
                        </span>
                        <span className="flex items-center gap-1">
        ...
        <div className="bg-white border border-[#121212]/5 rounded-xl p-3.5">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-light mt-0.5">
                        {meeting.dayConfig.startTime} – {meeting.dayConfig.endTime} ·{" "}
                        {meeting.dayConfig.mode === "PHYSICAL" ? "In-person" : "Online"}
                    </p>
                </div>
...
function PrayerSection() {
    ...
    const selectedMeetingIds = new Set(
        (prayerStatus?.mySelections ?? []).map((m) => m.id)
    );
    ...
    if (programsError) {
        return <p className="pt-3 text-xs text-red-500">{programsError}</p>;
    }

    if (programs.length === 0) {
        return <p className="pt-3 text-xs text-gray-400 font-light">No prayer programmes available.</p>;
    }
    ...
    return (
        <div className="pt-3 space-y-4">
            ...
            {/* Status banner */}
            {prayerStatus && (
                <div className={`text-xs px-3 py-2.5 rounded-xl border font-light ${prayerStatus.windowOpen
            ...
            {selectError && (
                <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">{selectError}</p>
            )}
...
export const ProfilePage = () => {
    ...
    // Mount the announcement push poller — departmentId from worker profile
    const departmentId = profile?.workerProfile?.department?.id;
    useAnnouncementPush({ departmentId, intervalMs: 60_000 });
                ...
                <div className="px-6 mt-8 max-w-md mx-auto space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white border border-[#121212]/5 rounded-2xl divide-y divide-[#121212]/5 shadow-sm overflow-hidden">
                            ...
                            <AccordionItem
                                id="account_details"
                                active={activeSection === "account_details"}
                            ...
                            <AccordionItem
                                id="serving_rota"
                                active={activeSection === "serving_rota"}
                                onToggle={toggleSection}
                                icon={Calendar}
                                label="My Serving Rota"
                            >
                            ...
                            <AccordionItem
                                id="tax_statements"
                                active={activeSection === "tax_statements"}
                                onToggle={toggleSection}
                                icon={HeartHandshake}
                                label="Tax Statements & Receipts"
                            >
                                <div className="pt-3 text-xs text-gray-600 font-light">
                            ...
                            <AccordionItem
                                id="report_incident"
                                active={activeSection === "report_incident"}
                                onToggle={toggleSection}
                                icon={AlertTriangle}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white border border-[#121212]/5 rounded-2xl divide-y divide-[#121212]/5 shadow-sm overflow-hidden">
                            ...
                            {/* Notifications — simplified, no toggle */}
                            <AccordionItem
                                id="notifications"
